...
namespace FargowiltasCrossmod.Core.Calamity.Globals
{
    ...
    public class CalDLCItemBalance : GlobalItem
    {
        public static float BalanceChange(Item item)
        {
            ...
            // Melee
            if (item.type == ItemType<IridescentExcalibur>()) return 1f;
            ...
            if (item.type == ItemType<RedSun>()) return 1f;
            if (item.type == ItemType<GaelsGreatsword>()) return 1f;
            // Ranged
            ...
            // Magic
            if (item.type == ItemType<Apotheosis>()) return 1f;
            if (item.type == ItemType<StaffofBlushie>()) return 1f;
            if (item.type == ItemType<Eternity>()) return 1f;
            if (item.type == ItemType<TheDanceofLight>()) return 1f;
            if (item.type == ItemType<RainbowPartyCannon>()) return 1f;
            if (item.type == ItemType<Fabstaff>()) return 1.2f;
            // Summoner
            if (item.type == ItemType<AngelicAlliance>()) return 1f;
            if (item.type == ItemType<FlamsteedRing>()) return 1f;
            if (item.type == ItemType<TemporalUmbrella>()) return 1f;
            if (item.type == ItemType<Endogenesis>()) return 1f;
            if (item.type == ItemType<Metastasis>()) return 1f;
            if (item.type == ItemType<UniverseSplitter>()) return 1f;
            if (item.type == ItemType<ProfanedSoulCrystal>()) return 1f;
            // Rogue
            if (item.type == ItemType<NanoblackReaper>()) return 1f;
            if (item.type == ItemType<ScarletDevil>()) return 1f;
            if (item.type == ItemType<TheAtomSplitter>()) return 1f;
            if (item.type == ItemType<Sacrifice>()) return 1f;
